    ...
    updateProgressInterval = setInterval(() => {
        ...
        .then(data => {
            ...
            if (data.status === 'ready') {
                // Update button to Install when download completes
                const row = document.querySelector(`tr[data-update-id="${updateId}"]`);
                if (row) {
                    const actionsCell = row.querySelector('td:last-child');
                    if (actionsCell) {
                        actionsCell.innerHTML = `<a class="btn btn-sm btn-success" href="#" onclick="installUpdate(${updateId}); return false;">Install</a>`;
                    }
                }
                stopProgressTracking();
...
function startInstallTracking(updateId) {
    ...
    updateProgressInterval = setInterval(() => {
        ...
        .then(data => {
            ...
            // Update logs in terminal
            updateTerminalLogs(updateId);
...
function showTerminal(updateId) {
    ...
    terminal.id = 'update-terminal';
    terminal.innerHTML = `
        <div class="terminal-container">
            <div class="terminal-header">
                <h4>Installation Progress - Update ID: ${updateId}</h4>
                <button class="terminal-close" onclick="hideTerminal()">&times;</button>
            </div>
            <div class="terminal-body" id="terminal-logs">
                <pre>Initializing installation...</pre>
            </div>
        </div>
    `;
...
function updateTerminalLogs(updateId) {
    fetch(`/admin/app/systemupdate/${updateId}/installation-logs/`)
    .then(response => response.json())
    .then(data => {
        if (data.status === 'success') {
            const logsElement = document.getElementById('terminal-logs');
            if (logsElement) {
                logsElement.innerHTML = `<pre>${data.logs || 'Waiting for logs...'}</pre>`;
