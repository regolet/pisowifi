...
class UpdateInstallService:
    ...
    def create_backup(self):
        ...
        try:
            self._log("Starting backup creation...")
            backup_name = f"backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            backup_dir = os.path.join(self.backup_path, backup_name)
            
            # Create backup directory
            os.makedirs(backup_dir, exist_ok=True)
            self._log(f"Created backup directory: {backup_name}")
    ...
    def install_update(self):
        ...
        try:
            self._log(f"Starting installation of update {self.update.Version_Number}")
            ...
            self._log("Update files extracted successfully")
            ...
            self._log("Update files copied successfully")
            ...
            self._log(f"Update {self.update.Version_Number} installed successfully!")
    ...
    def _run_post_install_tasks(self):
        ...
        try:
            ...
            if result.stdout:
                # Log only summary to avoid too much output
                lines = result.stdout.strip().split('\n')
                if lines:
                    self._log(f"Static files: {lines[-1]}")
            
            self._log("Post-installation tasks completed")
            
        except subprocess.CalledProcessError as e:
            self._log(f"Post-install task failed: {e}", 'WARNING')
            if e.stderr:
                self._log(f"Error details: {e.stderr}", 'WARNING')
